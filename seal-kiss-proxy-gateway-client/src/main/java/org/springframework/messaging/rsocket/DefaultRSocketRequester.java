...
final class DefaultRSocketRequester implements RSocketRequester {
	...
	private class DefaultRequestSpec implements RequestSpec {
		...
		@Override
		@SuppressWarnings("unchecked")
		public <T> Mono<Tuple2<T, Map<String, Object>>> retrieveMetadataMono(ParameterizedTypeReference<T> dataTypeRef) {
			Mono<Payload> payloadMono = rsocketClient.requestResponse(getPayloadMono());

			ResolvableType dataType = ResolvableType.forType(dataTypeRef);
			Decoder<?> decoder = strategies.decoder(dataType, dataMimeType);
			...
			return payloadMono.flatMap(payload -> {
				...
				Mono<T> data;
				if (isVoid(dataType)) {
					data = (Mono<T>) Mono.empty();
				} else {
					data = (Mono<T>) Mono.just(this.retainDataAndReleasePayload(payload))
							.map(dataBuffer -> decoder.decode(dataBuffer, dataType, dataMimeType, EMPTY_HINTS));
				}
				return Mono.zip(data, Mono.just(metadataMap));
			});
		}

		@Override
		public <T, V> Mono<Tuple2<T, V>> retrieveMetadataMono(Class<T> dataType, String metadataKey, Class<V> metadataType) {
			return retrieveMetadataMono(ResolvableType.forClass(dataType), metadataKey, ResolvableType.forClass(metadataType));
		}

		@Override
		public <T, V> Mono<Tuple2<T, V>> retrieveMetadataMono(ParameterizedTypeReference<T> dataTypeRef, String metadataKey, ParameterizedTypeReference<V> metadataTypeRef) {
			return retrieveMetadataMono(ResolvableType.forType(dataTypeRef), metadataKey, ResolvableType.forType(metadataTypeRef));
		}

		@SuppressWarnings("unchecked")
		private <T, V> Mono<Tuple2<T, V>> retrieveMetadataMono(ResolvableType dataType, String metadataKey, ResolvableType metadataType) {
			Mono<Payload> payloadMono = rsocketClient.requestResponse(getPayloadMono());
			...
			return payloadMono.flatMap(payload -> {
				Map<String, Object> metadataMap = metadataExtractor.extract(payload, metadataMimeType);
				V v = (V)metadataMap.get(metadataKey);
				Mono<V> metaData;
				if(null == v) {
					metaData = Mono.empty();
				} else {
					metaData = Mono.just(v);
				}
...
//				Mono<T> data;
//				if (isVoid(dataType)) {
//					data = (Mono<T>) payloadMono.then();
//				} else {
//					data = (Mono<T>) payloadMono.map(this::retainDataAndReleasePayload)
//							.map(dataBuffer -> decoder.decode(dataBuffer, dataType, dataMimeType, EMPTY_HINTS));
//				}
//				return Mono.zip(data, metaData);
//			});
